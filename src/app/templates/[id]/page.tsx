            ...
            <div className="grid grid-cols-1 lg:grid-cols-[1fr_380px] gap-16 items-start">
              ...
              <div className="pb-16">
                <div className="flex gap-2.5 mb-5 flex-wrap">
                  ...
                  <span className="px-3 py-1 rounded-full text-[10px] font-body uppercase tracking-wider border border-white/[0.07] text-mid bg-white/[0.05]">
                    Updated Jan 2025
                  </span>
                </div>

                <h1 className="font-display font-black text-[clamp(40px,5vw,64px)] leading-none tracking-[-0.03em] mb-5">
                  {t.name}
                  <br />
                ...
                <div className="flex gap-3 flex-wrap mb-10">
                  ...
                  <Link href="#" className="inline-flex items-center gap-2 px-7 py-4 rounded-full border border-white/[0.07] text-muted font-display font-semibold text-[13px] tracking-[0.05em] uppercase hover:text-white hover:border-white/[0.13] transition-all duration-200">
                    Live Preview ↗
                  </Link>
                </div>

                <div className="flex items-center gap-6 pt-8 border-t border-white/[0.07] flex-wrap">
                  {t.stats.map(s => (
                    <div key={s.label} className="flex items-center gap-2 text-[12px] text-muted font-body">
                      <strong className="text-white font-body">{s.num}</strong> {s.label}
                    </div>
                  ))}
              ...
              <div className="sticky top-20 pb-8">
                <div className="border border-white/[0.07] rounded-xl overflow-hidden bg-bg-2">
                  {/* Preview */}
                  <div
                  ...
                  {/* Body */}
                  <div className="p-6">
                    <div className={`font-display font-black text-[32px] mb-1 ${t.badge === "free" ? "text-acid" : "text-white"}`}>
                      {t.price}
                    </div>
                    <p className="text-[11px] text-muted font-body mb-5">
                    ...
                    <Link href="#" className="flex justify-center items-center gap-2 w-full px-5 py-3 rounded-full border border-white/[0.07] text-muted font-display font-semibold text-[11px] tracking-[0.05em] uppercase hover:text-white hover:border-white/[0.13] transition-all duration-200">
                      Live Preview ↗
                    </Link>
                    ...
                    <div className="flex flex-col gap-2.5 mb-5">
                      {["Mobile-first & responsive","Core Web Vitals 100","SEO meta management","Sanity Studio included","TypeScript support","Full documentation"].map(f => (
                        <div key={f} className="flex items-center gap-2.5 text-[12px] text-mid font-body">
                    ...
                    <div className="flex flex-col gap-2.5">
                      {t.stack.map(s => (
                        <div key={s} className="flex items-center gap-3 text-[12px] text-muted font-body">
                          <div className="w-7 h-7 rounded-lg bg-bg-4 border border-white/[0.07] flex items-center justify-center text-[13px]">▲</div>
                          {s}
                        </div>
                      ))}
                    </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
              {t.features.map((f) => (
                <div key={f.title} className="flex gap-4 p-6 bg-bg border border-white/[0.07] rounded-xl">
                  ...
                  <div>
                    <div className="font-display font-bold text-[14px] text-white mb-2">{f.title}</div>
                    <p className="text-[12px] text-muted font-body leading-[1.65]">{f.desc}</p>
                  </div>
                </div>
              ))}
            </div>
          ...
          <div className="max-w-7xl mx-auto">
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-20 items-center">
              <div>
                ...
                <p className="text-muted text-[13px] font-body leading-[1.8] mb-8">Full documentation included. Step-by-step setup guide from clone to deployed.</p>
              ...
              <div className="border border-white/[0.07] rounded-xl overflow-hidden">
                {[
                  ...
                  <div key={i} className="px-6 py-5 bg-bg-2 border-b border-white/[0.07] last:border-0">
                    <div className="text-[10px] text-muted font-body mb-2 uppercase tracking-wider">{step}</div>
        ...
        <section className="py-20 px-8 md:px-12 border-t border-white/[0.07] bg-bg-2">
          <div className="max-w-7xl mx-auto">
            <div className="flex justify-between items-center mb-10">
              ...
              <Link href="/templates" className="inline-flex items-center gap-2 px-5 py-2.5 rounded-full border border-white/[0.07] text-muted font-display font-semibold text-[11px] tracking-[0.05em] uppercase hover:text-white hover:border-white/[0.13] transition-all duration-200">
                Browse All →
              </Link>
            </div>
            <div className="grid grid-cols-1 md:grid-cols-3 gap-5">
              {[
                ...
                <Link href="/templates" key={m.name} className="group border border-white/[0.07] rounded-xl overflow-hidden hover:border-acid-border transition-all duration-200">
                  <div className="h-32 flex items-center justify-center text-4xl" style={{ background: m.bg }}>{m.emoji}</div>
                  <div className="p-5 bg-bg-2">
                    <div className="font-display font-bold text-[15px] text-white mb-1">{m.name}</div>
                    <div className="text-[12px] text-muted font-body mb-3">Sanity Template</div>
                    <span className={`px-3 py-1 rounded-full text-[10px] font-body uppercase tracking-wider border ${m.price === "Free" ? "bg-acid-dim text-acid border-acid-border" : "bg-white/[0.05] text-mid border-white/[0.07]"}`}>
                      {m.price}
                    </span>
                  </div>
                </Link>
